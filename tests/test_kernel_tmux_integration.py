...
pytestmark = pytest.mark.skipif(
    ...
    reason="tmux/psmux kernel integration tests require a compatible backend and CHAITYA_RUN_TMUX_TESTS=1",
)
...
class TestKernelTmuxInteractive:
    async def test_interactive_roundtrip_through_kernel(self) -> None:
        ...
        try:
            ...
            start_py = await kernel.dispatch(f'session send-input {session_name} "{python_cmd}" --newline')
